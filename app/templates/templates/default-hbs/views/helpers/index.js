...
module.exports = function() {
	 ...
	 * ==============================================
	 * 
	 * Also used in the default landmarkjs-hbs theme
	 */
	
	// ### Date Helper
	// A port of the Ghost Date formatter similar to the landmarkjs - jade interface
	//
	//
	// *Usage example:*
	// `{{date format='MM YYYY}}`
	...
	// ### Category Helper
	// Ghost uses Tags and Landmark uses Categories
	...
	_helpers.isAdminEditorCSS = function(user, options) {
		...
		if (typeof(user) !== 'undefined' && user.isAdmin) {
			output = cssLinkTemplate({
				href: "/landmark/styles/content/editor.min.css"
			});
		}
		return new hbs.SafeString(output);
	};
	...
	_helpers.isAdminEditorJS = function(user, options) {
		...
		if (typeof(user) !== 'undefined' && user.isAdmin) {
			output = scriptTemplate({
				src: '/landmark/js/content/editor.js'
			});
		}
		return new hbs.SafeString(output);
	};
	...
	_helpers.adminEditableUrl = function(user, options) {
		var rtn = landmark.app.locals.editable(user, {
			'list': 'Post',
			'id': options
		});
		return rtn;
	...
	// To support the limit src url strings used by LandmarkJS default theme
	...
	// ### Content Url Helpers
	// LandmarkJS url handling so that the routes are in one place for easier
	...
	* */
	...
	_helpers.paginationNavigation = function(pages, currentPage, totalPages, options){
		...
		// '...' will be added by landmark if the pages exceed 10
		_.each(pages, function(page, ctr){
			...
			// if '...' is sent from landmark then we need to override the url
			if(page === '...'){
	...
	//  LandmarkJS supports a message interface for information/errors to be passed from server
